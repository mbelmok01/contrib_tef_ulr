...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="//mets:metsHdr">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="CREATEDATE"><xsl:value-of select="$dateTimeCourante"/></xsl:attribute>
	...
	<xsl:template match="//mets:dmdSec">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="CREATED"><xsl:value-of select="$dateTimeCourante"/></xsl:attribute>
			<xsl:apply-templates select="*" />
		</xsl:copy>
	</xsl:template>
